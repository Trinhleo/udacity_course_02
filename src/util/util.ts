...
export async function filterImageFromURL(inputURL: string, tmpDir: string): Promise<string> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      await photo
        ...
        .write(tmpDir + outpath, (img) => {
          resolve(tmpDir + outpath);
        });
    } catch (error) {
      reject(error);
    }
...
export function cleanUpLocalFiles(tmpDir: string) {
  readdir(tmpDir, (err, files) => {
    if (err) {
      console.log(err);
    }
    const filesPath = files.map(file => tmpDir + file);
